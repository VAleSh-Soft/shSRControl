...
- [Работа с кнопками, библиотека srButton](#работа-с-кнопками-библиотека-srbutton)
...
* доработаны примеры в части работы с кнопками;
...
Опрос кнопок ведется самим модулем в методе `tick()`, отлавливаются события **BTN_DOWN**, но ничто не мешает использовать это и другие события кнопок модуля в своих целях. Однако, здесь есть один нюанс - для корректной и надежной работы с кнопками нельзя допускать множественные опросы кнопки в пределах одного прохода `loop()`. А т.к. метод `getButtonState()` для каждой кнопки вызывается в методе `tick()` модуля, то для работы следует использовать метод `getLastState()` кнопок. Например:
```
void loop()
{
