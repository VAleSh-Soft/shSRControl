...
Для работы с тактовыми кнопками модулей используется библиотека [shButton](https://github.com/VAleSh-Soft/shButton). Опрос кнопок ведется самим модулем в методе `tick()`, отлавливая события **BTN_DOWN**, однако это не мешает использовать это и другие события кнопок модуля в своих целях. Однако, в этом вопросе есть один нюанс - для корректной и надежной работы с кнопками не нужно допускать множественного опроса кнопки в пределах одного прохода `loop()`. А т.к. метод `getButtonState()` для каждой кнопки вызывается в методе `tick()` модуля, то для работы следует использовать метод `getLastState()` кнопок. Например:
```
void loop()
{
  switch_control.tick(); // здесь кнопки опрашиваются

  // ==== а здесь работаем с кнопками сами =========

  switch (btn1.getLastState()) // поэтому остается только получить результат опроса и использовать его
  {
  case BTN_LONGCLICK:
...
Для подробного описания методов работы с библиотекой `shButton` смотрите ее файл **readme.md**, ссылка на гитхаб в следующем заголовке.
