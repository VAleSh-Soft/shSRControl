...
В библиотеку данные о локальных/удаленных реле передаются в виде массивов:
...
shRelayData relays[];
```
Примеры инициализации данных и заполнения массивов можно посмотреть в примерах в папке **examples**.
...
Экземпляры модуля требуют инициализации с передачей в нее массива реле и их количества:
```
void init(shSwitchData *_switch_array, uint8_t _switch_count);
```
Здесь:
- *_switch_array - массив данных локальных реле;
- _switch_count -  количество реле в модуле;
```
void init(shRelayData *_relay_array, uint8_t _relay_count);
```
Здесь:
- *_relay_array - массив данных удаленных реле;
- _relay_count -  количество выключателей в модуле;
