...
* в связи с этим изменены примеры;
...
```
Здесь:
- `relayName` - имя реле (идентификатор), по которому к нему будет обращаться выключатель;
...
Если вы планируете использовать Web-интерфейс, то нужно дополнительно вызвать метод
```
...
```
Загрузка сохраненных параметров выполняется автоматически.
