...
private:
  ...
  uint16_t _debounce_timeout = 50u;       // интервал подавления дребезга контактов, мс
  uint16_t _longclick_timeout = 1000u;    // интервал удержания кнопки нажатой, мс
  uint16_t _dblclck_timeout = 300u;       // интервал двойного клика, мс
  uint8_t _longclick_mode = LCM_ONLYONCE; // режим удержания кнопки;
  uint16_t _interval_of_serial = 100u;    // интервал следования события BTN_LONGCLICK, если установлен режим LCM_CLICKSERIES, мс
...
public:
  ...
  bool isSecondButtonPressed(srButton &_but, uint8_t btn_state = BTN_DOWN);
  ...
  // установка режима обработки удержания кнопки нажатой; возможные варианты - LCM_CONTINUED, LCM_ONLYONCE или LCM_CLICKSERIES
  void setLongClickMode(uint8_t longclickmode);

  // установка интервала удержания кнопки (значение по умолчанию 500 мс);
